...
function p8_drop_file(e)
{
    ...
    if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0])
    {
        ...
        reader.onload = function (event)
        {
            ...
            e.stopPropagation();
            e.preventDefault();
            var codo_command = 9; // read directly from p8_dropped_cart with libb64 decoder
        };
...
function p8_update_button_icons()
{
    ...
    for (let i = 0; i < els.length; i++)
    {
        ...
        var new_str = '<img width=24 height=24 style="pointer-events:none" src="'+p8_gfx_dat[index]+'">';
...
function pico8_buttons_event(e, step)
{
    ...
    if (num == 0 && typeof(pico8_mouse) !== 'undefined') 
        pico8_mouse[2] = 0;  //  no active touches: release mouse button from anywhere on page. (maybe redundant? but just in case)
...
function p8_update_layout()
{
    ...
    var el = document.getElementById("p8_menu_buttons");
...
function p8_run_cart()
{
    ...
    if (p8_touch_detected)
    {
        var el = document.getElementById("body_0");
        var el2 = document.getElementById("p8_frame_0");
        if (el && el2)
        {
            el.style.display="none";
    ...
    // show touch elements
    var els = document.getElementsByClassName('p8_controller_area');
    for (let i = 0; i < els.length; i++)
        els[i].style.display="";
...
{
    var el = (typeof codo_textarea === 'undefined') ? document.getElementById("codo_textarea") : codo_textarea;
    if (el)
    {
        el.focus();
        el.select();
